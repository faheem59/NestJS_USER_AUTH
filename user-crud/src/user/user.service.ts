...
export class UserService {
    ...
    async update(id: number, updateUserData: UpdateUserDto): Promise<UpdateUserResponse> {
        try {
            ...
            return { message: SUCCESS_MESSAGES.USER_UPDATED, user: plainToClass(User, updatedUser) };
        } catch (error) {
            if (error instanceof NotFoundException) {
                throw error;
            }
    ...
    async addPermissionsToUser(userId: number, permissionNames: string[]): Promise<User> {
        return this.permissionRepository.addPermissionsToUser(userId, permissionNames);
    }

    async removePermissionsFromUser(userId: number, permissionNames: string[]): Promise<User> {
