...
export class AuthService {
    ...
    async createAdmin(adminData: CreateUserDto): Promise<CreateUserResponse> {
        try {
            ...
            await this.rabbitmqService.sendMessage('user created successfully', plainToClass(User, admin));
    ...
    async validateUser(userData: LoginDto): Promise<LoginUserResponse> {
        ...
        try {
            ...
            return { message: SUCCESS_MESSAGES.USER_LOGGEDIN, user: plainToClass(User, user), accessToken };
        } catch (error) {
    ...
    async validateRefreshToken(refreshToken: string): Promise<RefreshToken> {
        try {
            const tokenRecord = await this.refreshTokenRepository.findOne({ where: { token: refreshToken }, relations: ['user', 'user.role'] });
